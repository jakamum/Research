...
# this version uses fewer words and a symbolic text along with different colours to the previous trials
...
def spiral_text1(canvas, text1, x, y, radius, start_angle, font_size, font_type):
    ...
    for i, char in enumerate(text1):
        ...
        x1 = x + math.cos(angle + char_angle) * char_radius
        y1 = y + math.sin(angle + char_angle) * char_radius
        canvas.create_text(x1, y1, text=char, fill="black", font=(font_type, font_size))
    radius += 0.3
    return radius
...
root.title("Animated Expanding Text Black Hole")
